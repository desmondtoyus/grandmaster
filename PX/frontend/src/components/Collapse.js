    ...
    return (
      <div className={`collapse__wrapper ${this.state.class} ${Class}`}>
        <div onClick={this.toggle} className='collapse__title'>
          {this.state.icon}
          <p><b>{this.props.title}</b>{this.props.errIcon ? <Icon name='warning circle' size='large' color='red' />: null } <ChevronDownIcon/></p>
        </div>
        <Collapse
          isOpen={this.state.collapse || this.props.open}
