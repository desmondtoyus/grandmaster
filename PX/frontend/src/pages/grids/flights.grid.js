...
import { listFlights, listPausedFlights, readFlightTag, deleteFlight, listActiveFlights, listInactiveFlights, listDisabledFlights, activateFlight, deactivateFlight, pauseFlight, disableFlight, changeFlights, changeFlight, getSupplyList, listSupply, checkIflight } from '../../redux/actions/flight.actions';
...
class FlightsGrid extends Component {
  state={
    tab:''
  }
  componentWillMount() {
  ...
  handleTabs = name => {
    ...
    let master = activeUser.scope_account.is_zone_master;
    const payload = { master, searchTerm, sortBy, currentPage, sortDirection, pageChunk };
    if (match.params.id) {
      payload.id = Number(match.params.id);
    }
    switch(name) {
  ...
  renderTableBody = () => {
    ...
    return flights.map((item, index) => {
      return (
        <tr key={item.id}>
          <td >{item.id} {(item.demand_source_type === "rtb") ? (<Icon style={{ color: '#20C0E7' }} name="external" /> ) : (null)}{ item.is_retargeted ? (<Icon style={{ color: '#20C0E7' }} name="target" /> ): (null) } </td>
          <td >
            <Popup
                ...
                {!loader && flights.length ? <div>
                  <Table responsive className='table--bordered dashboard__table-crypto'>
                    <thead>
                      ...
                      <th >Flight <Icon color={nameSort === "sort" ? "black" : "blue"} name={nameSort} id="name" onClick={this.handleSort} style={cursorStyle} /></th>
...
export default withRouter(connect(mapStateToProps, {readFlightTag, listSupply, readActiveUser, listFlights, deleteFlight, listPausedFlights, listActiveFlights, listInactiveFlights, listDisabledFlights, activateFlight, deactivateFlight, pauseFlight, disableFlight, changeFlights, changeFlight, modalStateChange, getSupplyList, checkIflight })(FlightsGrid));
