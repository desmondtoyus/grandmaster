...
class DomainListsGrid extends Component {
  componentDidMount() {
    ...
    const payload = { listType: 'domain', searchTerm, currentPage, sortBy, sortDirection, pageChunk };
    this.props.listDomainLists(payload);
  }

  createDomainList = () => {
    this.props.history.push(`/ui/list/create/new`);
  };
  ...
  searchDomainLists = (event) => {
    ...
    const payload = { listType: 'domain', sortBy, sortDirection, pageChunk, searchTerm: event.target.value, currentPage: 1 };
    this.props.listDomainLists(payload);
  };
  ...
  handleSort = (event) => {
    ...
    const payload = { listType: 'domain', searchTerm, currentPage, pageChunk, sortBy: event.target.id, sortDirection: sortDirection === 'asc' ? 'desc' : 'asc' };
    this.props.listDomainLists(payload);
  };

  editDomainList = (id) => {
    this.props.history.push(`/ui/list/update/${id}`);
  };

  cloneDomainList = (id) => {
    this.props.history.push(`/ui/list/create/${id}`);
  };
  ...
  handlePagination = (currentPage) => {
    ...
    let payload = { listType: 'domain', searchTerm, sortBy, sortDirection, pageChunk, currentPage };
    this.props.listDomainLists(payload);
  };
    ...
    return lists.map(item => {
      return (
        <tr key={item.id}>
          ...
          <td>
            <Popup
              trigger={<Link to={`/ui/list/update/${item.id}`}>{item.name}</Link>}
              content={item.name}
              flowing
            />
          </td>
