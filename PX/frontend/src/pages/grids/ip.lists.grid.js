...
class DomainListsGrid extends Component {
    componentDidMount() {
        ...
        const payload = { listType: 'ip_address', searchTerm, currentPage, sortBy, sortDirection, pageChunk };
        this.props.listDomainLists(payload);
    }

    createDomainList = () => {
        this.props.history.push(`/ui/ip/create/new`);
    };
    ...
    searchDomainLists = (event) => {
        ...
        const payload = { listType: 'ip_address', sortBy, sortDirection, pageChunk, searchTerm: event.target.value, currentPage: 1 };
        this.props.listDomainLists(payload);
    };
    ...
    handleSort = (event) => {
        ...
        const payload = { listType: 'ip_address', searchTerm, currentPage, pageChunk, sortBy: event.target.id, sortDirection: sortDirection === 'asc' ? 'desc' : 'asc' };
        this.props.listDomainLists(payload);
    };

    editDomainList = (id) => {
        this.props.history.push(`/ui/ip/update/${id}`);
    };

    cloneDomainList = (id) => {
        this.props.history.push(`/ui/ip/create/${id}`);
    };
    ...
    handlePagination = (currentPage) => {
        ...
        let payload = { listType: 'ip_address', searchTerm, sortBy, sortDirection, pageChunk, currentPage };
        this.props.listDomainLists(payload);
    };
        ...
        return lists.map(item => {
            return (
                <tr key={item.id}>
                    ...
                    <td>
                        <Popup
                            trigger={<Link to={`/ui/ip/update/${item.id}`}>{item.name}</Link>}
                            content={item.name}
                            flowing
                        />
                    </td>
    ...
    render() {
        ...
        const { itemStyle, dimmerStyle, cursorStyle, divStyle } = styles;
                        ...
                        <div className='tabs tabs--bordered-bottom'>
                            <div className='tabs__wrap'>
                                ...
                                <Nav tabs>
                                    ...
                                    <NavItem>
                                        <NavLink onClick={this.renderList.bind(null, '/ui/lists')}
                                            >
                                            Domain Lists
                    </NavLink>
                                    </NavItem>
                                    ...
                                    <NavItem>
                                        <NavLink onClick={this.renderList.bind(null, '/ui/apps')}>
                                            APP Lists
                    </NavLink>
                                    </NavItem>
