...
import { isURLValid, parseQuery, isAllowed } from "../functions";
import { Select, Input, TextArea, Button, Checkbox, Dimmer } from 'semantic-ui-react';
import { Breadcrumb, BreadcrumbItem, Alert, Card, CardBody, Col, Row, } from 'reactstrap';
import classNames from 'classnames';
import { Link } from 'react-router-dom';
import Dropzone from 'react-dropzone';
import { connect } from 'react-redux';
import { domainListChange, uploadBundleList, createDomainList, resetDomainListErrors, resetDomainListReducer, readDomainList, updateDomainList, cloneDomainList } from '../redux/actions/lists.actions';
...
class DomainList extends Component {
    ...
    onListDrop = (accepted, rejected) => {
        ...
        if (accepted[0].type !== "text/plain") {
            ...
            this.props.uploadBundleList(payload);
        }
    };
    ...
    cancelAction = () => {
        this.props.resetDomainListReducer();
        this.props.history.push(`/ui/bundles`);
    };
    ...
    isInputValid = () => {
        ...
        if (name.length < 6) {
            arr.push('Bundle ID list name shall be at least 6 characters');
            this.props.domainListChange({ prop: 'errorName', value: true });
        }
        if (name.length > 100) {
            arr.push('Bundle ID list name shall be at most 100 characters');
            this.props.domainListChange({ prop: 'errorName', value: true });
        }

        if (!(name.length > 100) && !(name.length < 6) && !isNaN(name)) {
            arr.push('Bundle ID name cannot all be numbers');
            this.props.domainListChange({ prop: 'errorName', value: true });
        }

        if (domains === "" && !uploadedDomains.length && !list.length) {
            arr.push('Please enter or upload Bundle IDs');
        ...
        return (
            <div className={"sub-content"}>
                <Breadcrumb tag="nav">
                    ...
                    <BreadcrumbItem ><Link to={`/ui/bundles`} className='link-a'>Bundle ID</Link></BreadcrumbItem>
                    <BreadcrumbItem active tag="span" >{match.params.status === "update" ? 'Update Bundle ID' : 'Create Bundle ID'}</BreadcrumbItem>
                </Breadcrumb>
                        ...
                        <CardBody>
                            <form className='form'>
                                <div className='form__half'>
                                    <h5>Bundle ID Details</h5>
                                    <div className='form__inside' >
                                        <div>
                                            {name ? <label className={classNames('bwa-select-label', { 'bwa-floated': name })} > Bundle ID Name</label> : <div style={{ marginTop: '10px' }}></div>}
                                            <Input type="text" fluid placeholder='Bundle ID Name' name="name" value={name} onChange={this.handleChange} error={errorName} />
                                        </div>
                                    </div>
                                    ...
                                    {match.params.id !== "new" ? <div className='form__inside' >
                                        <div>
                                            {list.length ? <label className={classNames('bwa-select-label', { 'bwa-floated': list })} > Bundle ID</label> : <div style={{ marginTop: '10px' }}></div>}
                                            <Select fluid placeholder="Bundle ID" value={list ? list : ['']} multiple options={this.getOptions()}
                                    ...
                                    <br />
                                    <div className='form__inside' >
                                        {match.params.id === "new" ? <h5><br />Enter multiple Bundle ID comma or newline separated</h5> : <h5>Add new Bundle ID comma or newline separated</h5>}
                                        <div>
                                            {domains ? <label className={classNames('bwa-select-label', { 'bwa-floated': name })} > Add new Bundle ID</label> : <div style={{ marginTop: '10px' }}></div>}
                                            <TextArea placeholder='Add new Bundle ID' name="domains" rows='6' value={domains} onChange={this.handleChange} />
                                        </div>
                                    </div>

                                    <div className='form__inside' >
                                        <h5>Upload Bundle ID</h5>
                                ...
                                <div className='form__half'>
                                    ...
                                    {fail ? <Alert color='danger'> Failed to create a bundle list. Please check your inputs or try again later</Alert> : null}
                                </div>
                            </form>
...
export default withRouter(connect(mapStateToProps, { domainListChange, uploadBundleList, createDomainList, resetDomainListErrors, resetDomainListReducer, readDomainList, updateDomainList, cloneDomainList })(DomainList));