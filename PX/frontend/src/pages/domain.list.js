...
import { domainListChange, uploadList, createDomainList, resetDomainListErrors, resetDomainListReducer, readDomainList, updateDomainList, cloneDomainList } from '../redux/actions/lists.actions';
...
class DomainList extends Component {
  ...
  onListDrop = (accepted, rejected) => {
    ...
    if (accepted[0].type !== "text/plain") {
      ...
      this.props.uploadList(payload);
    }
  };

  cancelAction = () => {
    this.props.resetDomainListReducer();
    this.props.history.push(`/ui/lists`);
  };
  ...
  isInputValid = () => {
    ...
    if (name.length < 6) {
      arr.push('Domain list name shall be at least 6 characters');
      this.props.domainListChange({ prop: 'errorName', value: true });
    }
    if (name.length > 100) {
      arr.push('Domain list name shall be at most 100 characters');
      this.props.domainListChange({ prop: 'errorName', value: true });
    }

    if (!(name.length > 100) && !(name.length < 6) && !isNaN(name)) {
      arr.push('Domain name cannot all be numbers');
      this.props.domainListChange({ prop: 'errorName', value: true });
    }

    if (domains === "" && !uploadedDomains.length && !list.length) {
      arr.push('Please enter or upload domains');
    ...
    return (
      <div className={"sub-content"}>
        <Breadcrumb tag="nav">
          ...
          <BreadcrumbItem ><Link to={`/ui/lists`} className='link-a'>Domain List</Link></BreadcrumbItem>
          <BreadcrumbItem active tag="span" >{match.params.status === "update" ? 'Update Domain List' : 'Create Domain List'}</BreadcrumbItem>
        </Breadcrumb>
            ...
            <CardBody>
              <form className='form'>
                <div className='form__half'>
                  <h5>Domain List Details</h5>
                  <div className='form__inside' >
                    <div>
                      {name ? <label className={classNames('bwa-select-label', { 'bwa-floated': name })} > Domain List Name</label> : <label></label>}
                      <Input type="text" fluid placeholder='Domain List Name' name="name" value={name} onChange={this.handleChange} error={errorName} />
                    </div>
                  </div>
                  ...
                  {match.params.id !== "new" ? <div className='form__inside' >
                    <div>
                      {list.length ? <label className={classNames('bwa-select-label', { 'bwa-floated': list })} > Domain List</label> : <label></label>}
                      <Select fluid placeholder="Domain List" value={list ? list : ['']} multiple options={this.getOptions()}
...
<br/>
                  <div className='form__inside' >
                    {match.params.id === "new" ? <h5><br/>Enter multiple Domain List comma or newline separated</h5> : <h5>Add new Domain List comma or newline separated</h5>}
                    <div>
                      {domains ? <label className={classNames('bwa-select-label', { 'bwa-floated': name })} > Add new Domain List</label> : <label ></label>}
                      <TextArea placeholder='Add new Domain List' name="domains" rows='6' value={domains} onChange={this.handleChange} />
                    </div>
                  </div>

                  <div className='form__inside' >
                    <h5>Upload Domain List</h5>
                ...
                <div className='form__half'>
                  ...
                  {fail ? <Alert color='danger'> Failed to create a domain list. Please check your inputs or try again later</Alert>:null}
                </div>
              </form>
...
export default withRouter(connect(mapStateToProps, { domainListChange, uploadList, createDomainList, resetDomainListErrors, resetDomainListReducer, readDomainList, updateDomainList, cloneDomainList })(DomainList));