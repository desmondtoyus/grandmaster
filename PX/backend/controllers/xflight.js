...
function generateVastDocument(data, advertiser_id) {
  ...
  if (process.env.NODE_ENV == "development" || !process.env.NODE_ENV) {
    creativeDomain = "http://video.bwacdn.com";
  } else {
    creativeDomain = "https://video.pilotxcdn.com";
  }
...
exports.read = function(req, res, next) {
  console.log("Flight number=", req.body.id);
  if (validators.id(req.body.id)) {
    models.flights
      .findOne({
        where: {
...
exports.videoUpload = function(req, res, next) {
  if (utils.isAllowed(req.user, actions.UPLOAD_CREATIVES, null)) {
    ...
    syncVideoToCDN(name, suffix, req.files.upload_file.path).then(response => {
      if (response) {
        ffprobe(
          req.files.upload_file.path,
          { path: ffprobeStatic.path },
          function(err, info) {
            ...
            const payload = {
              ...
              duration: Math.floor(Number(info.streams[0].duration)),
...
async function syncVideoToCDN(name, suffix, path) {
  let otherSuffix = suffix == "mp4" ? "webm" : "mp4";
  console.log("FIRST CONVERSION = ", suffix);
  console.log("SECOND CONVERSION = ", otherSuffix);
  try {
    let sftp1 = new Client();
    if (process.env.NODE_ENV == "development" || !process.env.NODE_ENV) {
      ...
      await sftp1
        .connect({
          host: "www.bwatest.com",
          username: "ubuntu",
          privateKey: fs.readFileSync("./keys/cdnbox.pem")
        })
        .then(() => {
          sftp1
            .put(path, `/var/www/html/assets/cdn/video/${name}.${suffix}`)
            .then(success => {
              // do somethings
              ffmpeg(`${path}`)
                ...
                // temporarily hold the converted file before moving to cdn
                .saveToFile(`./video/${name}.${otherSuffix}`);
              setTimeout(function() {
                sftp1
                  .put(
                    `./video/${name}.${otherSuffix}`,
                    `/var/www/html/assets/cdn/video/${name}.${otherSuffix}`
                  )
                  .then(success => {
                    shell.rm("-rf", path);
                    // remove the converted file from the folder
                    shell.rm("-f", "video/*");
                    console.log("Clone upload Success");
                  })
                  .catch(err => {
        ...
        if (campaign) {
          if (utils.isAllowed(req.user, actions.READ_CAMPAIGN, campaign)) {
            return listCampaignsAndAdvertisers(
              ...
              .then(response => {
                console.log("ADVERTISER ID", campaign.advertiser.id);
                if (response.success) {
                  const payload = {
                    campaigns: [],
                    advertisers: [],
