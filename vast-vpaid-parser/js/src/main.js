...
((_G) => {
  ...
  if (typeof window.Vast !== 'undefined') {
    if (DEBUG) {
      FW.log('Vast constructor already exists - no need to load it twice - exiting');
    }
    return;
  }
  /* module:ends */
  ...
  window.Vast = function (id, params) {
    ...
    if (DEBUG) {
      FW.log('creating new Vast instance');
    ...
    FW.ajax(this.adTagUrl, this.params.ajaxTimeout, true, this.params.ajaxWithCredentials).then((data) => {
      ...
      try {
        ...
        if (DEBUG) {
          ...
          FW.log(xml);
          document.getElementById('xmlLogs').innerText =format(data);
        }
      } catch (e) {
        FW.trace(e);
        // in case this is a wrapper we need to ping for errors on originating tags
