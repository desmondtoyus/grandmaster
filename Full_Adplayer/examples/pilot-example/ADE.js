...
var Player = function (id, vastTag, inArticle) {
    ...
    this.init = function () {
        ...
        Player.prototype.initFromStart = function () {
            if (!this.initialized) {
                this.init();
        ...
        Player.prototype.init = function () {
            ...
            this.playerz.ima.initializeAdDisplayContainer();
        };
...
Player.prototype.onAdEvent = function (event) {
    console.log('CHECKING FOR AD EVENRTS')
...
Player.prototype.onAdEvent = function (event) {
    if (event.type == google.ima.AdEvent.Type.CONTENT_PAUSE_REQUESTED) {
        ...
        console.log(' HERE Ad event: ' + event.type);
    }
};

Player.prototype.onPlaylistItemClick = function (event) {
    if (!this.linearAdPlaying) {
        ...
        this.playerz.ima.setContentWithAdTag(
            this.contents[event.target.id],
            null,
            false);
        this.playerz.poster(this.posters[event.target.id]);
        this.playerz.ima.requestAds();
    }
