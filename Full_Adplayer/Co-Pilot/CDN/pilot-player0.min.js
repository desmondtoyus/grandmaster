...
Player.prototype.adsManagerLoadedCallback=function(){var events=[google.ima.AdEvent.Type.ALL_ADS_COMPLETED,google.ima.AdEvent.Type.CLICK,google.ima.AdEvent.Type.COMPLETE,google.ima.AdEvent.Type.FIRST_QUARTILE,google.ima.AdEvent.Type.LOADED,google.ima.AdEvent.Type.MIDPOINT,google.ima.AdEvent.Type.PAUSED,google.ima.AdEvent.Type.STARTED,google.ima.AdEvent.Type.THIRD_QUARTILE];for(var index=0;index<events.length;index++){this.playerz.ima.addEventListener(events[index],this.onAdEvent.bind(this))}};Player.prototype.onAdEvent=function(event){let endingPlay=document.getElementById(this.id);let logoDiv=document.createElement('div');logoDiv.classList.add("logostyle");let logoImg=document.createElement('img');logoImg.src='https://player.pilotxcdn.com/CoPilot.png';logoDiv.appendChild(logoImg);let controlDiv=this.id+'_ima-controls-div';let dataView=endingPlay.getAttribute('data-view');if(endingPlay.parentNode.classList.contains('in_article')&&!document.getElementById('placeholder'+this.id)){let placeholder=document.createElement('div');placeholder.id='placeholder'+this.id;endingPlay.parentNode.parentNode.insertBefore(placeholder,endingPlay.parentNode)}
if(dataView!=='mobile_web'){document.getElementById(controlDiv).appendChild(logoDiv)}
if(event.type=='loaded'){endingPlay.parentNode.id='pilot-'+this.id;if(endingPlay.hasAttribute("muted")){videojs(this.id).ima.getAdsManager().setVolume(0)}
if(dataView=='mobile_web'){document.getElementById(this.id+'_ima-play-pause-div').style.left='75%';document.getElementById(this.id+'_ima-mute-div').style.left='85%'
...
if(endingPlay.parentNode.classList.contains('slider')){endingPlay.parentNode.style.visibility='visible';endingPlay.style.visibility='visible';pilotSlider(this.id);let style=window.getComputedStyle(endingPlay);let playerHeight=style.getPropertyValue('height');let parentHeight=parseInt(playerHeight)+20;endingPlay.parentNode.style.height=parentHeight+'px'}
...
function checkScroll(){if(document.getElementsByClassName("pilot-player")[0]){for(let i=0;i<videos.length;i++){var playVideo=videos[i];let playId=videos[i].getAttribute('id');var x=playVideo.offsetLeft,y=playVideo.offsetTop,w=playVideo.offsetWidth,h=playVideo.offsetHeight,r=x+w,b=y+h,visibleX,visibleY,visible;visibleX=Math.max(0,Math.min(w,window.pageXOffset+window.innerWidth-x,r-window.pageXOffset));visibleY=Math.max(0,Math.min(h,window.pageYOffset+window.innerHeight-y,b-window.pageYOffset));visible=visibleX*visibleY/(w*h);if(document.getElementById(playId).parentNode.classList.contains('stuck')&&inPicture(document.getElementById('placeholder'+playId))){document.getElementById(playId).parentNode.classList.remove("stuck")}
if(visible>fraction){let indexOfPlayer=playList.indexOf(playId);if(!(document.getElementById(playId).parentNode.classList.contains('pilot-checker'))&&!(document.getElementById(playId).parentNode.classList.contains('stuck'))&&playId&&indexOfPlayer>-1){play(playId);playList.splice(indexOfPlayer,1);console.log('PLAY')}
if((!document.getElementById(playId).classList.contains('pilot-checker'))&&(document.getElementById(playId).parentNode.classList.contains('stuck'))&&inPicture(document.getElementById('placeholder'+playId))){document.getElementById(playId).parentNode.classList.remove('stuck')}
playVideo.style.offsetHeight=realHeight[i]+"px";playVideo.style.width=realWidth[i]+"px";document.getElementById(playId).classList.add("pilot-checker");if(document.getElementById(playId).parentNode.classList.contains('stuck')&&inPicture(document.getElementById('placeholder'+playId))){document.getElementById(playId).parentNode.classList.remove("stuck");document.getElementsByClassName('in_article')[i].style.offsetHeight=realHeight[i]+"px"}}else{if(!document.getElementById(playId).parentNode.classList.contains('in_article')||playList.indexOf(playId)==-1){playList.push(playId)}
if((document.getElementById(playId).classList.contains('pilot-checker'))&&(document.getElementById(playId).parentNode.classList.contains('in_article'))&&!inPicture(document.getElementById('placeholder'+playId))){document.getElementById(playId).parentNode.classList.add("stuck");document.getElementById(playId).parentNode.classList.remove("pilot-checker")}
